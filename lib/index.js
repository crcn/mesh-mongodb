...
extend(Adapter.prototype, EventEmitter.prototype, {
  ...
  load: function(collection, operation, onRun) {
    var query = operation.query || { _id: operation.data[this.idProperty] };
    if (operation.multi) {
  ...
  update: function(collection, operation, onRun) {
    var query = operation.query || { _id: operation.data[this.idProperty] };
    if (operation.multi) {
      collection.update(query, { $set: operation.data }, onRun);
    } else {
  ...
  _collection: function(operation) {
    return this._collections[operation.collection] || (this._collections[operation.collection] = this.target.collection(operation.collection));
  },
